...
def course_options():
    with open('utilities/2025_offerings_details.txt') as csv:
        return csv.read()
...
Ensure the sum of suggested coursework hours is LESS THAN OR EQUAL TO 19 hours.
...
Prioritize courses in the order of: meeting GEC requirments > meeting DEG requirements > other classes. 
Do not recommend a course listed as a capstone requirement unless the student meets the requirements to graduate after taking the schedule you recommend.
If a student is in Honors, they must take their appropriate (lowest numbered and unsatisfied) HON prefix course each semester.
Suggestions should be offered as a list and contain the suggested course prefix, number, title, hours, and reason the student should take the course.
Ensure all suggested are listed in the courses list. DO NOT SUGGEST COURSES THE STUDENT HAS ALREADY TAKEN. HOUR CACULATIONS MUST BE CORRECT.

Once you have finished making an advisement, calculate the student's semester and year of graduation if they take 15 hours of coursework a semester.
Assume the current semester is Fall 2024.
